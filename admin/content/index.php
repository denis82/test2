<?
  $title_page = "Управление контентом сайта";
  ...
  for($number_cmd = 0; $number_cmd < $count_cmd; $number_cmd++){
    if($p_cmd=="new"){
      ...
      echo edit_element($new);
    	...
    	if($e_id){
	      echo show_tree_elements($e_id);
    	}else {
    		$e_id = get_main_element($common_options["content_te_id"]);
	      echo show_tree_elements($e_id);
    	}
      ...
      if($e["sort"]>1){
    	...
    	echo show_tree_elements($parent_el["e_id"]);
